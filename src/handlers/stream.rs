use std::{fs::File, io::{self, Read}, marker::PhantomData, os, sync::{mpsc::Sender, Arc}, thread::{self, JoinHandle}};

use bytes::Bytes;
use parking_lot::Mutex;

use crate::jupyter::messages::{iopub::{self, IopubBroacast}, multipart::Multipart, Header, Message, Metadata};
...
pub struct StreamHandler {
    ...
    // iopub_tx: Sender<Multipart>, // moved into the thread
}

impl StreamHandler {
    pub fn start(stream_name: iopub::StreamName, iopub_tx: Sender<Multipart>) -> io::Result<(Self, File)> {
        // TODO: construct a Self, create a pipe, start a reader thread, return the writer as a file
        ...
        thread::Builder::new().name(format!("{} reader", stream_name.as_ref())).spawn(move || {
